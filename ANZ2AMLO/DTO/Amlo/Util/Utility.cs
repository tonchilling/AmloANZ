...
namespace DTO.Util
{
	...
	public class Utility
	{
        ...
        static string[] shortMonths = { "Jan", "Feb", "Mar", "Apr", "May", "June","Jul","Aug","Sep","Oct","Nov","Dec" };
       // static string[] shortMonths = { "1st", "2", "Mar", "Apr", "May", "June", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
        public Utility()
		{
			//
        ...
        public static void WriteLogAppend(string filePath,string fileName,string text)
        {
           // File.CreateText
            if (HaveDirectory(filePath))
            {
                if (HaveFile(filePath + fileName))
                {
                    ...
                    using (StreamWriter w = File.CreateText(filePath + fileName))
                    {
                        Log(text, w);
                    }
        ...
        public static string SaveImage(string localFullPath,string destpath,string fileName)
        {

            string imgPath = "";
         // Construct a bitmap from the button image resource.
            Bitmap bmp1 = new Bitmap(string.Format("{0}", localFullPath));
            if (HaveDirectory(destpath))
            {
                imgPath = string.Format("{0}\\{1}", destpath, fileName);
